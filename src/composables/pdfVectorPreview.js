import { createVectorGraphicsContext, multiplyCanvasMatrices } from './pdfVectorGraphics.js';
...
export function createSvgDocument(prefix = `cv-vector-${++previewSerial}`) {
  let state = { matrix: [1, 0, 0, 1, 0, 0], clips: [], fill: '#000', stroke: '#000', fillAlpha: 1, strokeAlpha: 1, width: 1, cap: 'butt', join: 'miter', miter: 10, dash: [], dashOffset: 0 };
  const stack = [], definitions = [], elements = [];
  let path = '';
  const matrix = () => `matrix(${state.matrix.join(' ')})`;
  const color = (value) => Array.isArray(value) ? `rgb(${value.join(',')})` : value;
  ...
  const doc = {
    ...
    text(record, paint) {
      ...
      const baseline = { bottom: 'text-after-edge', top: 'text-before-edge', middle: 'central', hanging: 'hanging' }[paint.textBaseline] || 'alphabetic';
...
export async function renderVectorPreview({ recording, canvas, pages, links, pageWidth, pageHeight, task }) {
  const result = [];
