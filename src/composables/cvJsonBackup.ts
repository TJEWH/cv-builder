...
function hasCvStateShape(value: unknown): value is LegacyCvState {
...
export function createCvJsonBackup(state: LegacyCvState, exportedAt = new Date()) {
  const data: LegacyCvState = JSON.parse(JSON.stringify(state));
  return {
 ...
 */
export function parseCvJsonBackup(text: string): LegacyCvState {
