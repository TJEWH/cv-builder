...
import { createPdfRenderTask, createPreviewRenderSlot } from '../src/composables/pdfRenderTask.js';
...
test('a newer preview aborts the previous render at its next checkpoint', async () => {
  ...
  let resume;
  const task = createPdfRenderTask(oldSignal, { yieldTurn: () => new Promise(resolve => { resume = resolve; }) });
