...
import { applyPdfPageBreaks } from '../src/composables/pdfPageBreaks.js';

function fixture(style, top, bottom) {
  const inserted = [];
  const element = { getBoundingClientRect: () => ({ top: top + 50, bottom: bottom + 50 }), parentNode: { insertBefore: (node, before) => inserted.push({ height: node.style.cssText, before }) }, nextSibling: null };
...
test('locked items crossing a boundary are moved using root-relative coordinates', async () => {
  ...
  await applyPdfPageBreaks(root, 100, { checkpoint() {} });
...
test('layout changes are synchronized immediately after inserting a spacer', async () => {
  ...
  const counts = [];
  await applyPdfPageBreaks(root, 100, { checkpoint() {} }, () => counts.push(inserted.length));
