...
import { normalizeContentState } from '../src/composables/contentLayout.js';
import { useSectionVersions } from '../src/composables/useSectionVersions.js';

function fixture(t) {
  const makeVersion = (name, customIds) => {
    const data = {
  ...
  return { versions, state, saved, saves, statuses, selectedId, configurations,
    fail: (value) => { failSave = value; }, activeChanges: () => activeChanges };
}
...
test('edits to alternate versions persist together without touching the active CV or switching it', (t) => {
  ...
  versions.sectionState('jobs').experience.jobs = [{ title: 'Different story' }];
  versions.sectionState('shared').customSections[0].entries[0].title = 'Different project';
  versions.sectionState('skills').sidebarSections[0].items.push({ name: 'New skill' });
