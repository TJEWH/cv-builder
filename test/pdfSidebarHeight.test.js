...
import { fitDeferredSidebarToContent } from '../src/composables/pdfSidebarHeight.js';
...
test('content-height deferred sidebars release the float below their content', () => {
  ...
  fitDeferredSidebarToContent(root);
...
test('normal-flow and missing sidebars need no height override', () => {
  assert.doesNotThrow(() => fitDeferredSidebarToContent({ querySelector: () => null }));
});
