...
import { capturePdfLinks, vectorPageLinks } from '../src/composables/pdfVectorLinks.js';
...
test('capture links preserves multi-line rectangles but excludes unsafe actions and empty boxes', () => {
  ...
  const anchor = (href, rects = [rect]) => ({ href, getClientRects: () => rects });
  const root = {
  ...
  const capture = capturePdfLinks(root);
